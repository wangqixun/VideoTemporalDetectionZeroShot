...
img_norm_cfg = dict(
    mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375], to_bgr=False)  # RBG
train_pipeline = [
...
test_pipeline = [
    dict(type='DecordInit'),
    dict(
        type='SampleFrames',  # 从头到尾分num_clips段，依次在中间每frame_interval帧取1帧，共clip_len帧
        clip_len=32,
        frame_interval=2,
        num_clips=4,
        test_mode=True),
    dict(type='DecordDecode'),
    dict(type='Resize', scale=(-1, 224)),  # 短边224， 保留h/w
    dict(type='ThreeCrop', crop_size=224),  # 从图片中裁剪左中右（横屏）或者上中下（竖屏）三个矩形图片
    dict(type='Flip', flip_ratio=0),
    dict(type='Normalize', **img_norm_cfg),
    dict(type='FormatShape', input_format='NCTHW'),
    dict(type='Collect', keys=['imgs', 'label'], meta_keys=[]),
