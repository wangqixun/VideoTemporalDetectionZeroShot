...
class BaseRecognizerZeroShot(nn.Module, metaclass=ABCMeta):
    """Base class for recognizers.
    ...
    def __init__(self,
        ...
        if backbone['type'].startswith('mmcls.'):
            ...
            if not self.use_vision_feature:
                self.backbone = builder.build_backbone(backbone)
            else:
                self.backbone = None
            if not self.use_nlp_feature:
                self.nlp_backbone = builder.build_backbone(nlp_backbone)
            else:
                self.nlp_backbone = None
        ...
        self.vision_language_decoder = builder.build_backbone(decoder_backbone)
    ...
    def forward(self, data_batch=None, return_loss=True, **kwargs):
        """Define the computation performed at every call."""
        if return_loss:
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        losses = self(data_batch=data_batch, return_loss=True, **aux_info)
